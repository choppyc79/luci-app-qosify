...
### Advanced
**Backup** lists both files with size, modification time and a Download button. **Restore** is a file picker per file; Upload & Apply replaces the chosen files (validated, 64 KB cap, binary rejected). **Mapping Files** edits the `defaults` list in `config defaults` — the files qosify loads mappings from, which `qosify.init` passes to the daemon as the config `files` array. Entries are added and removed as a list and written back as `list defaults` lines in place, leaving the rest of the file untouched; Save & Apply then pushes the config so the daemon picks the new list up. Because `qosify.init` word-splits and glob-expands the list in a shell `for` loop, an entry must be an absolute path with no whitespace and no shell metacharacters, and a glob like `/etc/qosify/*.conf` is expanded at push time rather than by qosify. **Maintenance** has Check Devices, `ubus call qosify check_devices`, which re-runs the daemon's own `qosify_iface_check()` so a device that appeared after qosify started is picked up without rebuilding the qdiscs. **Defaults** resets both files back to qosify defaults after a confirmation.
