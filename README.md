...
- **Polling follows LuCI** — every tab ticks at `luci.main.pollinterval` instead of a fixed 10 s
...
### Status
The detailed `qosify-status` output with CAKE qdisc statistics for egress and ingress; the per-interface summary is in the Overview Service section. The tab fetches as soon as it is opened and the scroll position survives a refresh. Polled at LuCI's refresh interval (`luci.main.pollinterval`, 5 seconds by default) only while the tab is open; a tick that would overlap the last one is skipped.
...
`luci-app-qosify` is in [openwrt/luci](https://github.com/openwrt/luci/tree/master/applications/luci-app-qosify) master (#8833, #9019, #9054) and ships in snapshots. Since #9054 merged on 21 September 2026 the package carries the 3.0.x code, file for file the same as this installer's apart from the installer's `?v=<VERSION>` stylesheet suffix, so `migrate` moves you to the package without losing anything. 3.0.6 to 3.0.11 are bug fixes on top of that, submitted to openwrt/luci master as one PR; until it merges the package is behind this installer.
